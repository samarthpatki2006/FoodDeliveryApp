...
import { getInitialRestaurants } from "../../api/customer.api.js";
...
const Badge = ({ children, variant = "default" }) => {
  ...
  return (
    <span
      className={`inline-block rounded-full px-2.5 py-0.5 text-[0.65rem] font-bold uppercase tracking-wide whitespace-nowrap ${variants[variant]}`}
    >
      {children}
    </span>
  );
...
const Home = () => {
  ...
  useEffect(() => {
    (async () => {
      ...
      try {
        ...
        const res1 = await getInitialRestaurants();
        if (res.status < 300) setAddresses(res.data.data);
        if (res1.status < 300) setRestaurants(res1.data.data);
      } catch {
      } finally {
  ...
  const handleFetchNearby = async () => {
    if (!location.latitude) {
      setError("Get your live location first.");
      return;
    }
    setError("");
    ...
    try {
      ...
      setError(
        e?.response?.data?.message || "No restaurants found in this radius.",
      );
    } finally {
  ...
  const isVeg = (item) =>
    item.is_veg || item.category?.toLowerCase().includes("veg");
      ...
      <div className="mx-auto max-w-6xl">
        ...
        <div className="space-y-5">
          <Section icon={<Search size={18} />} title="Search">
            ...
            {tab === "saved" && (
              <div className="flex flex-wrap items-center gap-3">
                ...
                >
                  <option value="">
                    {isFetchingAddr ? "Loading…" : "All saved addresses"}
                  </option>
                  {addresses.map((a) => (
            ...
            {tab === "nearby" && (
              <div className="flex flex-wrap items-center gap-3">
                ...
                <select
                  ...
                  className="rounded-xl border border-gray-100 bg-white px-3 py-3 text-sm text-gray-700 outline-none focus:border-orange-300 focus:ring-2 focus:ring-orange-100 transition"
                >
          ...
          {activeList.length > 0 && (
            <Section
              icon={<Store size={18} />}
              title={`${activeList.length} restaurant${activeList.length !== 1 ? "s" : ""}${tab === "nearby" && location.latitude ? ` within ${radius} km` : ""}`}
            >
                  ...
                  >
                    ...
                    <div className="p-3.5">
                      ...
                      <div className="flex items-center justify-between gap-2 flex-wrap">
                        ...
                        {rest.cuisine_type && (
                          <Badge variant="cuisine">{rest.cuisine_type}</Badge>
                        )}
                      </div>
                      {rest.delivery_time_minutes && (
                        <p className="mt-2 flex items-center gap-1 text-xs text-gray-400">
                          ...
                          {rest.min_order_amount
                            ? ` · ₹${rest.min_order_amount} min order`
                            : ""}
                        </p>
                      )}
                    </div>
                  ...
                  {groupedMenu.map((item) => (
                    ...
                    >
                      ...
                      <div className="p-2.5">
                        ...
                        {item.description && (
                          <p className="mt-0.5 text-[0.71rem] leading-snug text-gray-500 line-clamp-2">
                            {item.description}
                          </p>
                        )}
...
const InfoChip = ({ label, children, wide }) => (
  <div
    className={`rounded-xl bg-gray-50 px-3 py-2.5 text-sm text-gray-700 ${wide ? "col-span-2" : ""}`}
  >
    <p className="mb-0.5 text-[0.63rem] font-bold uppercase tracking-wider text-gray-400">
      {label}
    </p>
    {children}
  </div>
